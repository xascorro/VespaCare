...
ADMIN_CHAT_ID = int(os.environ.get("ADMIN_CHAT_ID", "129347404"))

BASE_DIR = "/var/www/html/vespa"
...
VOICE_DIR = os.path.join(BASE_DIR, "docs", "voice")
...
def get_photo_inline_keyboard(doc_filename):
    return {
        "inline_keyboard": [
            [
                {"text": "📖 Entrada de Diario", "callback_data": f"doccat:{doc_filename}:diario"},
                {"text": "⛽ Ticket Repostaje", "callback_data": f"doccat:{doc_filename}:repostaje"}
            ],
            [
                {"text": "💶 Factura / Recambio", "callback_data": f"doccat:{doc_filename}:factura"},
                {"text": "📋 ITV / Seguro / Legal", "callback_data": f"doccat:{doc_filename}:legal"}
            ]
        ]
    }
...
Sincronizado en <b>Gastos</b> y <b>Stock Taller</b> en la PWA.""",
    ...
    # 5. DETECTAR ENTRADA DE DIARIO / BITÁCORA / REPARACIÓN (Ej: '/diario Hoy reparé la fuga del carburador...')
    is_diario_intent = (
        lower.startswith("/diario") or
        lower.startswith("/nota") or
        lower.startswith("/bitacora") or
        lower.startswith("diario:") or
        lower.startswith("reparacion:") or
        "reparé" in lower or
        "he reparado" in lower or
        "reparada" in lower or
        "reparado" in lower or
        "ajusté" in lower or
        "he ajustado" in lower or
        "he limpiado" in lower or
        "limpié" in lower or
        "he cambiado" in lower or
        "he montado" in lower
    )
    ...
    if is_diario_intent:
        ...
        # Extraer precio/coste si lo hubiera
        precio = 0.0
        price_match = re.search(r'([\d\.,]+)\s*(?:€|euros?|eur)\b', lower)
        if price_match:
            precio = float(price_match.group(1).replace(',', '.'))
        ...
        # Limpiar texto
        clean_text = re.sub(r'^(?:/diario|/nota|/bitacora|diario:|reparacion:)\s*', '', raw, flags=re.IGNORECASE).strip()
        
        # Categoría
        cat = "reparacion"
        if any(k in lower for k in ["mantenimiento", "revision", "aceite", "bujia", "filtro", "neumatico"]):
            cat = "mantenimiento"
        elif any(k in lower for k in ["ajuste", "carburador", "tornillo", "ralenti", "cable", "embrague"]):
            cat = "ajuste"
        elif any(k in lower for k in ["ruta", "viaje", "carretera", "paseo"]):
            cat = "ruta"
        elif any(k in lower for k in ["mejora", "accesorio", "luz", "led"]):
            cat = "mejora"

        title = clean_text.splitlines()[0] if clean_text else "Nota de Taller"
        if len(title) > 60:
            title = title[:57] + "..."
        ...
        if "diario" not in data or not isinstance(data["diario"], list):
            data["diario"] = []
        data["diario"].insert(0, diario_entry)
        save_data(data)

        photo_msg = f"\n📸 <b>Foto adjunta:</b> <code>{attached_doc}</code>" if attached_doc else ""
        send_message(
            f"""📖 <b>Entrada en Diario de a Bordo Registrada:</b>

🏷️ <b>Título:</b> {title}
📂 <b>Categoría:</b> {cat.upper()}
📍 <b>Kilometraje:</b> <code>{km} km</code>{photo_msg}
📝 <b>Notas:</b> {clean_text}

Sincronizado en la pestaña <b>Diario</b> de VespaCare PWA.""",
            chat_id,
            reply_markup=get_status_inline_keyboard()
        )
        return True
...
def handle_callback_query(cq):
    ...
    if data_cb == "cmd_estado":
    ...
    elif data_cb.startswith("doccat:") or data_cb.startswith("doccat_"):
        if ":" in data_cb:
            parts = data_cb.split(":")
            doc_file = parts[1]
            cat = parts[2]
        else:
            rest = data_cb[len("doccat_"):]
            if "_" in rest:
                doc_file, cat = rest.rsplit("_", 1)
            else:
                doc_file, cat = rest, "otro"
        ...
        if cat == "diario":
            if "diario" not in data or not isinstance(data["diario"], list):
                data["diario"] = []
            data["diario"].insert(0, {
                "id": f"d_tg_{int(time.time()*1000)}",
                "date": datetime.now().strftime("%Y-%m-%dT%H:%M"),
                "km": data.get("odometer", 0),
                "title": f"Foto de Taller / Reparación ({datetime.now().strftime('%d/%m/%Y')})",
                "category": "reparacion",
                "notes": "Foto subida desde Telegram Bot",
                "photos": [{
                    "filename": doc_file,
                    "original_name": doc_file,
                    "caption": "Subida vía Telegram"
                }],
                "cost": 0,
                "autoGenerated": False
            })
        ...
        dest_txt = "la pestaña <b>Diario</b> y el expediente documental" if cat == "diario" else "el expediente documental"
        edit_message_text(
            f"✅ <b>Documento Clasificado:</b> <code>{doc_file}</code>\n📂 <b>Categoría:</b> {cat.upper()}\n\nGuardado en {dest_txt} de VespaCare.",
            chat_id,
            msg_id,
            reply_markup=get_status_inline_keyboard()
        )
